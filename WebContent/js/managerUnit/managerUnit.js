	...
	$
		.confirm({
			...
			content : '<div><form id="updateUnitForm">'
				...
				+ '<label>机构代码：</label><input readonly="false" id="update_unit_num" name="unit.unit_num" class="form-control unitInput">'
				+ '<label>单位账号：</label><input readonly="false" id="update_unit_account" name="unit.unit_account" class="form-control unitInput">'
				+ '<label>联系人姓名：</label><input id="update_unit_contacts_name" name="unit.unit_contacts_name" class="form-control unitInput">'
				...
				$
					.ajax({
						...
						success : function(data) {
							...
							$('#update_unit_contacts_name').val(
								unitServer.unit_contacts_name);
						}
					})
			}
		})
...
function addUnit(event) {
	$.confirm({
		...
		content : '<div><form id="unitForm">'
			...
			+ '<label>联系人姓名：</label><input name="unit.unit_contacts_name" class="form-control unitInput">'
		...
		buttons : {
			...
			save : {
				...
				action : function() {
					...
					if (!/^1\d{10}$/.test($('#add_unit_phone').val())) {
						flag = false;
					}
					if (flag) {
						...
						formData.append('unit.unit_account', formData.get("unit.unit_num"));
						$.ajax({
